...
type Props = {
  ...
  text: string;
  likesCount: number;
  onLikeButtonClick?: (id: string, uid: string) => void;
};

const Review = memo(
  ({ avatarUrl, userName, date, text, likers, likesCount, id, uid, onLikeButtonClick }: Props) => {
    ...
    return (
      <S.Review>
        ...
        <S.MessageWrapper>
          <S.LeftWrapper>
            <LikeButton likesCount={likesCount} isActive={isLikeActive} onCheckboxChange={onLike} />
          </S.LeftWrapper>
          <S.Text>{text}</S.Text>
        </S.MessageWrapper>
      </S.Review>
